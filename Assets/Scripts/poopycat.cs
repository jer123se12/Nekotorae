...
public class poopyCat : basecat
{
    ...
    {
        Vector3 pred;
        Vector3 d = new Vector3(UnityEngine.Random.Range(-(width / 2), (width / 2)), 0, UnityEngine.Random.Range(-(length / 2), (length / 2)));
        pred=d;
        Ray check = new Ray(transform.position, destination-transform.position);
        RaycastHit hitData;
    ...
    override public void normalChange(float elp)
    {
        ...
        nextpooptime = elp + UnityEngine.Random.Range(minPoopTime, maxPoopTime);
    }
    float startmov=0;
    override public void normalUpdate(float elp)
    {
        ...
        {
            nextpooptime = elp + UnityEngine.Random.Range(minPoopTime, maxPoopTime);
            GameObject poo = GameObject.Instantiate(poop, pooplocation.position, pooplocation.rotation);
            //float rngsize=
            poo.transform.localScale=new Vector3(UnityEngine.Random.Range(0.5f, 1.5f)*15, UnityEngine.Random.Range(0.5f, 1.5f)*15, UnityEngine.Random.Range(0.5f, 1.5f)*15);
    ...
    {
        if ((launched == true||movingToward==true) && isEscaping == false)
        {
